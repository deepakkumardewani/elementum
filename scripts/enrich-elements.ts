...
const USER_AGENT = "periodic-table-explorer/1.0 (educational enrichment script; bun)"
...
async function main() {
  const elements = JSON.parse(readFileSync(ELEMENTS_PATH, "utf-8")) as Record<string, unknown>[]
  const staticData = JSON.parse(readFileSync(STATIC_DATA_PATH, "utf-8")) as StaticData
  ...
  // Spot-check Iron
  const iron = elements.find((e) => (e as { atomicNumber: number }).atomicNumber === 26) as Record<
    string,
    unknown
  >
