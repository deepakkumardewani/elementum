...
export const useElementStore = defineStore("element", () => {
  ...
  const displayHighlightSet = computed(() =>
    toolHighlightElements.value.size > 0 ? toolHighlightElements.value : highlightedElements.value,
  )
  ...
  function recomputeHighlighted() {
    ...
    const { tokens: searchTokens, plainText } = parseAdvancedSearchTokens(rawQuery)
    ...
    const matched = elements.value.filter((el) => {
      ...
      const matchesStructured =
        searchTokens.length === 0 || elementMatchesAdvancedTokens(el, searchTokens)
